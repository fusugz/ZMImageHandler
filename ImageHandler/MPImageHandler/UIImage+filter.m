...
+ (UIImage *)addFilterUsingCoreImage:(UIImage *)originImage filterName:(NSString *)filterName {
    ...
    CIFilter *sepiaTone = [CIFilter filterWithName:filterName
                                     keysAndValues:kCIInputImageKey, inputImage, @"inputIntensity", [NSNumber numberWithFloat:1.0], nil];
    
    CIImage *result = [sepiaTone outputImage];
    
    CIContext *coreImageContext = [CIContext contextWithOptions:nil];
    CGImageRef resultRef = [coreImageContext createCGImage:result fromRect:CGRectMake(0, 0, originImage.size.width, originImage.size.height)];
